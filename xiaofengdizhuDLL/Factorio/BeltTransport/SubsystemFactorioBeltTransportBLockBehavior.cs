...
namespace Game
{
    internal class SubsystemFactorioTransportBeltBLockBehavior : SubsystemBlockBehavior, IDrawable
    {
        ...
        public static int[] m_colorDrawScale = new int[] { 16, 8, 4 };
            ...
            try
            {
                if (drawOrder == 10)
                {
                    ...
                    foreach (FTBandItems block in m_blocks.Values)
                    {
                        ...
                        Vector3 FTBdisplayPosition = new Vector3((float)FTB.position.X + 0.5f, (float)FTB.position.Y + (FTB.slopeType.HasValue ? 0.5f : 0.01f), (float)FTB.position.Z + 0.5f);
                        if (Vector3.Distance(FTBdisplayPosition, camera.ViewPosition) < m_visibilityRange + 10)
                        {
                            ...
                            Vector3[] FTBdisplayVertices = new Vector3[4];
                            for (int i = 0; i < 4; i++)
                            {
                            ...
                            for (int side = 0; side <= 1; side++)
                            {
                                for (int process = 4; process >= 0; process--)
                                {
                                    ...
                                    if (item.value > 0)
                                    {
                                        ...
                                        Matrix matrix = new Matrix(1f, 0f, 0f, 0f, 0f, 1f, 0f, 0f, 0f, 0f, 1f, 0f, FTBdisplayPosition.X, FTBdisplayPosition.Y, FTBdisplayPosition.Z, 1f);
                                        ...
                                        else
                                        {
                                            if (FTB.slopeType.HasValue)
                                            {
                                                ...
                                                {
                                                    matrix.M42 += (float)process * 0.25f - 0.25f;
                                                }
                                                else
                                                {
                                                    matrix.M42 += 0.5f - (float)process * 0.25f;
                                                }
                                            }
                                            else
                                            {
                    ...
                    this.m_primitivesRenderer.Flush(camera.ViewProjectionMatrix, false, int.MaxValue);
                }
            }
            catch (Exception e)
            {
