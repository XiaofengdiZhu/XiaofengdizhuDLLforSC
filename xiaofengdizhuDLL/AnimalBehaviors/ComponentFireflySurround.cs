    ...
    class ComponentFireflySurround : Component, IDrawable
    {
        ...
        public void Draw(Camera camera, int drawOrder)
        {
            if (isLivingMode && drawOrder == 10)
            {
                ...
                Point2 playerCell = new Point2((int)playerPosition.X, (int)playerPosition.Z);
                ...
                if (m_subsystemSky.SkyLightIntensity < 0.4 && m_subsystemWeather.m_globalPrecipitationIntensity == 0 && !SubsystemWeather.IsPlaceFrozen(m_subsystemTerrain.Terrain.GetTemperature(playerCell.X, playerCell.Y), (int)playerPosition.Y))
                {
                    if (nowTime > m_lastSpawnTime + 1)
                    {
                        ...
                        Vector3 randomPosition = new Vector3(playerPosition.X + m_random.UniformFloat(-20f, 20f), 0, playerPosition.Z + m_random.UniformFloat(-20f, 20f));
                        Point2 cell = new Point2((int)randomPosition.X, (int)randomPosition.Z);
                ...
                foreach (Firefly firefly in m_fireFlies)
                {
                    ...
                    if (distance < 32)
                    {
                        ...
                        Vector3 hsv = new Vector3(firefly.hue, firefly.saturation, MathUtils.Clamp(0.85f + size * 10 - distance * 0.01f, 0f, 1f));
