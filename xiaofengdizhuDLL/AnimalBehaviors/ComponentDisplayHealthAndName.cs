...
namespace Game
{
    internal class ComponentDisplayHealthAndNameBehavior : Component, IDrawable
    {
        ...
        private readonly int[] m_drawOrders = new int[1];
        ...
        public void Draw(Camera camera, int drawOrder)
        {
            if (!m_isPlayer)
            {
                var vector = Vector3.Transform(m_componentCreature.ComponentBody.Position + Vector3.UnitY * m_height + new Vector3(0, 0.4f, 0), camera.ViewMatrix);
                var vector2 = Vector3.Transform(m_componentCreature.ComponentBody.Position + Vector3.UnitY * m_height + new Vector3(0, 0.2f, 0), camera.ViewMatrix);
                if (vector.Z < 0f)
                {
                    var color = Color.Lerp(Color.White, Color.Transparent, MathUtils.Saturate((vector.Length() - 16f) / 3f));
                    if (color.A > 6)
                    {
